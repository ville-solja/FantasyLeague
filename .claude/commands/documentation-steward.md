...
## Scope
- Covers: `markdown/feature_description/`, `markdown/user_stories.md`, `.env.example` vs `backend/` implementation
...
## Precondition check
Verify that `markdown/feature_description/` exists and contains at least one `.md` file, and that `backend/main.py` exists. If either is missing, report and stop.
...
For each significant endpoint group or background task in `main.py` and any routers, check whether it is covered by a `feature_description/*.md` file. Flag endpoint groups or subsystems with no corresponding spec.
