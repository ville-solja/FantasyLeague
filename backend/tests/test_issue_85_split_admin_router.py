...
class TestPreserveExistingTestCoverageThroughSplit:
    ...
    def test_full_suite_pass_skip_counts_match_pre_split_baseline(self):
        """`cd backend && python -m pytest tests/ -v` reports the same 526 passed / 10 skipped as before the split.
        ...
        plan-multi-admin-support's 19 new tests did after that, and the
        ingest-league-empty-response bug fix's 4 new tests did after that)."""
        ...
        assert "526 passed" in output, output[-3000:]
