...
class TestPreserveExistingTestCoverageThroughSplit:
    ...
    def test_full_suite_pass_skip_counts_match_pre_split_baseline(self):
        """`cd backend && python -m pytest tests/ -v` reports the same 663 passed / 10 skipped as before the split.
        ...
        plan-issue-101-schedule-fixtures-api's 9 new tests did after that, and
        plan-issue-107-card-team-logo-placeholder's 4 new tests did after that, and
        plan-issue-109-opendota-query-prioritization's 6 new tests did after that, and
        plan-issue-84-week-management-editing's 8 new tests did after that)."""
        ...
        assert "663 passed" in output, output[-3000:]
