...
class TestPreserveExistingTestCoverageThroughSplit:
    ...
    def test_full_suite_pass_skip_counts_match_pre_split_baseline(self):
        """`cd backend && python -m pytest tests/ -v` reports the same 522 passed / 10 skipped as before the split.
        ...
        plan-issue-89-fix-available-draws-count's 9 new tests did after that, and
        ...
        assert "522 passed" in output, output[-3000:]
