...
Shows the rarity drop percentages for the current draw pool, broken down by rarity (Common, Rare, Epic, Legendary) with a normalised percentage for each (e.g. 60%, 25%, 10%, 5%). Percentages come from the live `draw_rate_*` scoring weights and always sum to 100%. Below the rarity grid:
